...
int main(int nOUT, mxArray *pOUT[], int nINP, const mxArray *pINP[])
{
    int nPM,nPN;
    int nP, nC;
    int iP, iC;   
  ...
  if ((nINP==0)||(nINP!=4))
  {
    Printf("Detect points inside a polygonal region \n");
    Printf("InPolygon True for points inside or on a polygonal region.\n");
    Printf("\tIN = InPolygon(X,Y,XV,YV) returns a matrix IN the size of X and Y.\n");
    Printf("\tIN(p,q) = 1 if the point (X(p,q), Y(p,q)) is either strictly inside or\n");
    Printf("\ton the edge of the polygonal region whose vertices are specified by the\n");
    Printf("\tvectors XV and YV; otherwise IN(p,q) = 0.\n\n"); 
    Printf("\t[IN ON] = InPolygon(X,Y,XV,YV) returns a second matrix, ON, which is \n");
    Printf("\tthe size of X and Y. ON(p,q) = 1 if the point (X(p,q), Y(p,q)) is on\n"); 
    Printf("\tthe edge of the polygonal region; otherwise ON(p,q) = 0.\n\n");
    Printf("\t[IN ON IN_STRICT] = InPolygon(X,Y,XV,YV) returns a third matrix, IN_EX,  \n");
    Printf("\twhich is the size of X and Y. IN_EX(p,q) = 1 if the point (X(p,q), Y(p,q))\n"); 
    Printf("\tis strictly inside the polygonal region; otherwise IN_EX(p,q) = 0.\n");    
    Printf("\n\tThe polygon may be closed or not, i.e. the last point does have\n");     
    Printf("\tto be identical to the first one.\n");
    Printf("\n\tExample:\n");     
    Printf("\t[IN,ON,IN_STRICT] = InPolygon(0.6,0.5,[0 1 1 0],[0 0 1 1]);\n");
    Printf("\n\tTo check the efficiency of the proposed routine,\n\tyou can run this little script:\n");
    Printf("\t\tnpoints = 1e5;\n");
    Printf("\t\tL = linspace(0,2.*pi,6); xv = cos(L)';yv = sin(L)';\n");
    Printf("\t\txv = [xv ; xv(1)]; yv = [yv ; yv(1)];\n");
    Printf("\t\tx = randn(npoints,1); y = randn(npoints,1);\n");
    Printf("\t\ttic,in = inpolygon(x,y,xv,yv);\n");
    Printf("\t\tdisp(['Time needed by MatLab inpolygon routine = ' num2str(toc)])\n");
    Printf("\t\ttic,[points_in_on,points_on,points_in] = InPolygon(x,y,xv,yv);\n");
    Printf("\t\tdisp(['Time needed by InPolygon routine = ' num2str(toc)])\n");
    Printf("\t\tplot(xv,yv,x(in),y(in),'r+',x(~in),y(~in),'bo')\n");
    Printf("\n\tCredits: G. Jacquenot");
